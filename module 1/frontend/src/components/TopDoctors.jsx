    ...
    <div className='flex flex-col items-center gap-6 my-16 text-gray-800 md:mx-10'>
        ...
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 pt-5 px-2 sm:px-0">
            {doctors.slice(0,10).map((item, index) => (
                <div onClick={()=>navigate(`/appointment/${item._id}`)} key={index} className='flex flex-col items-center gap-2 border border-blue-300 rounded-xl overflow-hidden p-4 hover:translate-y-[-10px] transition-all duration-500 cursor-pointer'>
